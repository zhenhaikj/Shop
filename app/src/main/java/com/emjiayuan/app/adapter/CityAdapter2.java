...
public class CityAdapter2 extends BaseAdapter {
    ...
    public void setList(List<AreaModel> list) {
        ...
        try {
            for (int i = 0; i < list.size(); i++) {
                String currentStr = PinyinUtil.chineseToPinYinS(list.get(i).getCityName());
                String previewStr = (i - 1) >= 0 ? PinyinUtil.chineseToPinYinS(list.get(i - 1).getCityName())
                        : " ";
                if (!previewStr.equals(currentStr)) {//前一个首字母与当前首字母不同时加入HashMap中同时显示该字母
                    String name = PinyinUtil.chineseToPinYinS(list.get(i).getCityName());
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        try {
            currentStr = PinyinUtil.chineseToPinYinS(list.get(position).getCityName());
            previewStr = (position - 1) >= 0 ? PinyinUtil.chineseToPinYinS(list.get(position - 1)
                    .getCityName()) : " ";
        } catch (BadHanyuPinyinOutputFormatCombination badHanyuPinyinOutputFormatCombination) {
            badHanyuPinyinOutputFormatCombination.printStackTrace();
        }
